...
def contracts_handler(total_contracts, record, driver):
    ...
    while True:
        ...
        time.sleep(1)
        logging.info('[合同]%s：需要加载46/51/52条，已经加载%s条，等待' % (driver.title,len(rows)))
        rows = driver.find_elements_by_css_selector('section [class*="browse"]')
    logging.info('[合同]%s：需要加载46/51/52条，已经加载%s条' % (driver.title,len(rows)))
    ...
    if len(rows) == 46:
        record['关联框架合同编号'] = rows[6].get_attribute('textContent')
        record['备注'] = rows[7].get_attribute('textContent')
        record['是否未录入OA合同'] = rows[8].get_attribute('textContent')
        record['线下合同编号'] = rows[9].get_attribute('textContent')
        record['合同类别'] = rows[10].get_attribute('textContent')
        record['合同价'] = rows[11].get_attribute('textContent')
        record['硬件金额'] = rows[13].get_attribute('textContent')
        record['成本'] = rows[14].get_attribute('textContent')
        record['合同内容'] = rows[15].get_attribute('textContent')
        record['服务期限'] = [rows[16].get_attribute('textContent'),rows[17].get_attribute('textContent')]
        record['收款方式'] = rows[18].get_attribute('textContent')
        record['备注'] = rows[19].get_attribute('textContent')
        record['项目编号'] = rows[21].get_attribute('textContent')
        record['项目名称'] = rows[22].get_attribute('textContent')
        record['软件许可数'] = rows[23].get_attribute('textContent')
        record['纯服务合同'] = rows[24].get_attribute('textContent')
        record['运维服务'] = rows[25].get_attribute('textContent')
        record['产品'] = rows[26].get_attribute('textContent')
        record['产品分类'] = [rows[27].get_attribute('textContent'), rows[28].get_attribute('textContent')]
        record['业务来源'] = rows[29].get_attribute('textContent')
        record['业务需求'] = rows[30].get_attribute('textContent')
        record['行业类型'] = rows[31].get_attribute('textContent')
        record['所属大区'] = rows[32].get_attribute('textContent')
        record['签署收款金额'] = rows[33].get_attribute('textContent')
        record['到货收款金额'] = rows[34].get_attribute('textContent')
        record['初验收款金额'] = rows[35].get_attribute('textContent')
        record['终验收款金额'] = rows[36].get_attribute('textContent')
        record['保内尾款金额'] = rows[37].get_attribute('textContent')
        record['其他'] = rows[38].get_attribute('textContent')
        record['合同约定付款节点'] = rows[39].get_attribute('textContent')
        record['收款节点总计'] = rows[40].get_attribute('textContent')
        record['比例合计'] = rows[41].get_attribute('textContent')
        record['盖章人'] = rows[43].get_attribute('textContent')
        record['盖章状态'] = rows[44].get_attribute('textContent')
    else:
...
def weekly_reports_handler(total_weekly_reports, record, driver):
    ...
    for row in rows:
       ...
       #     tds = row.find_elements_by_css_selector('section [class*="browse"]')
        ...
        report['项目名称'] = tds[3].get_attribute('textContent')
...
def main():
    ...
    driver = login()
    time.sleep(5)
    driver.get('https://sl.xslp.com/index.php/Home/Project/index')
    time.sleep(5)
...
#            elif '销售合同' in subject or '销售审批单' in subject or 'PO合同' in subject:
